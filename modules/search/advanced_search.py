...
class AdvancedSearcher:
    ...
    def search_with_filtering(self, criteria: Dict[str, Any],
        ...
        results = self.search_multiple_sources(query, sources or ['people', 'email', 'social'])
    ...
    def _search_email(self, query: str) -> Dict[str, Any]:
        try:
            ...
            return {"source": "email", "query": query, "error": f"Búsqueda email: {e}"}

    def _search_social(self, query: str) -> Dict[str, Any]:
        """Búsqueda de perfiles sociales real usando el módulo socmint"""
        try:
            from . import socmint  # importar módulo con funciones, no instanciable

            username = query.strip()
            if not username:
                return {"source": "social", "query": query, "error": "Username vacío"}

            # Llamada a la función de búsqueda de perfiles sociales
            social_data = socmint.search_social_profiles(username=username)
    ...
    def _search_email(self, query: str) -> Dict[str, Any]:
        """Búsqueda de información de email usando emailint si el query es un email válido"""
        try:
            ...
            # Llamada a la función de búsqueda de email
            return emailint.search_email_info(email=query, user_id=user_id)
    ...
    def _search_dorks(self, query: str) -> Dict[str, Any]:
        try:
            ...
            google_api_key = config_manager.get_config(st.session_state.get('current_user_id'), 'google_api_key') or None
            google_cx = config_manager.get_config(st.session_state.get('current_user_id'), 'google_custom_search_cx') or None
        ...
        for source, data in results.items():
            if isinstance(data, dict) and 'results' in data:
                ...
                if filters.get('location_filter'):
                    filtered_data = [r for r in filtered_data if filters['location_filter'] in str(r.get('location', ''))]
