...
# -------------------------------------------------------------------
# ✅ COMPAT: breach_block.py dependía de esta función.
# La reintroducimos como wrapper para no romper imports.
# -------------------------------------------------------------------
def _render_card(card: Dict[str, Any]):
    """
    Compatibilidad con módulos que importan `_render_card` (p.ej. breach_block).
    Renderiza la tarjeta individual con el mismo HTML del grid.
    """
    st.markdown(_DORKS_CSS, unsafe_allow_html=True)
    st.markdown(f"<div class='q3-grid'>{_card_html(card)}</div>", unsafe_allow_html=True)

    # Detalles técnicos (opcional)
    with st.expander("Detalles técnicos", expanded=False):
        if card.get("pattern"):
            st.caption("Dork / patrón")
            st.code(card.get("pattern"))
        if card.get("query_used"):
            st.caption("Consulta ejecutada")
            st.code(card.get("query_used"))
        st.caption("URL")
        st.code(card.get("url") or "#")
...
def render_dorks_block(dorks_block: Dict[str, Any]):
    ...
    if not cards:
        st.info(
            ...
            "- Red/DNS/proxy/DNS (si estás en Streamlit Cloud, suele limitar scraping)\n"
        )
