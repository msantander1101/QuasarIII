...
# ------------------------------
# CSS: mismo look profesional que dorks_block
# ------------------------------
_GENERAL_CSS = """
<style>
/* Layout responsive */
...
# ------------------------------
# Helpers (mismos que en dorks_block)
# ------------------------------
...
def _render_card(card: Dict[str, Any]):
    ...
    entity_type = card.get("entity_type") or "page"
    ...
    # Para general_search no hay patrón/dork como tal, pero dejamos el panel por consistencia
...
# ------------------------------
# Flatten de general_web
# ------------------------------
def _flatten_general_results(general_block: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Recibe el bloque tal y como viene de AdvancedSearcher._search_general_web:
    ...
    Lo convertimos en una lista de "cards" uniformes para la UI.
    """
    flat: List[Dict[str, Any]] = []

    results = general_block.get("results") or {}
    raw = results.get("raw_results") or {}

    if not isinstance(raw, dict):
        return flat

    for engine_key, entries in raw.items():
        ...
        for e in entries:
            ...
            source = e.get("source") or "general_search"
            engine = engine_key

            ts = e.get("timestamp")
            published_at = _ts_to_str(ts)

            # Heurística sencilla para relevancia: confianza * 100
            try:
                rel_score = int(float(confidence) * 100) if confidence is not None else 60
            except Exception:
                rel_score = 60
            ...
            flat.append({
                ...
                # meta técnico (aquí no hay dork/patrón como tal)
...
# ------------------------------
# Render principal para el radar general
# ------------------------------
def render_general_block(general_block: Dict[str, Any]):
    """
    Renderiza el bloque de 'general_web' con el mismo formato visual que dorks_block.
    Espera que le pases results["general_web"] de advanced_search.
    """
    if not isinstance(general_block, dict):
        return

    # CSS (una vez)
    st.markdown(_GENERAL_CSS, unsafe_allow_html=True)

    # errores del wrapper
    for err in general_block.get("errors") or []:
        st.warning(f"General web: {err}")

    # errores internos del motor general_search
    inner_res = general_block.get("results") or {}
    for err in inner_res.get("errors") or []:
        st.warning(f"General web (inner): {err}")

    cards = _flatten_general_results(general_block)

    # Si no hay nada útil, no mostramos el bloque
    if not cards:
        return

    # Ordenación igual que en dorks: relevancia, riesgo, confianza, dominio
    ...
    st.markdown("### 🌐 Radar general web")
    st.caption(f"{len(cards)} resultados de motores públicos (ordenados por relevancia/confianza)")
